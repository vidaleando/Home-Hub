...
def display():
	...
	with canvas(device, background=image_composition()) as draw:
		...
		draw.multiline_text(((int(device.width - draw.textlength(msgTime, FontTime)))/2, device.height-16), msgTime, font=FontTime, align="center")
...
def Splash():
	if Args.SplashScreen:
		with canvas(device) as draw:
			...
			draw.multiline_text((45, 35), "Version : 1.0 -  By Vidal & Ewa", font=ImageFont.truetype("%s/resources/Skinny.ttf" % (os.path.dirname(os.path.abspath(inspect.getfile(inspect.currentframe())))),15), align="center")
